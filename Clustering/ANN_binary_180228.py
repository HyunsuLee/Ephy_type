...
"""
log_path = '/180228binary_full/bn_wrapper_test/'
...
for epoch in range(5000):
    ...
    sess.run(optimizer, feed_dict={X: trainX, Y: trainY, keep_prob: 0.5, is_training: 1})
    if (epoch % 5) == 0:
        train_acc, train_summ = sess.run([accuracy, summ], 
                        feed_dict={X: trainX, Y: trainY, keep_prob: 1.0, is_training: 1})
        train_writer.add_summary(train_summ, epoch)

        test_acc, test_summ = sess.run([accuracy, summ], 
                        feed_dict={X: testX, Y: testY, keep_prob: 1.0, is_training: 0})
