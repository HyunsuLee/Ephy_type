...
result_path = './180301_hyperparameter_test/10_2_Iline_short_fine.csv'
...
Y = tf.placeholder(tf.float32, [None, 8]) # binary E vs I class
...
for model in range(total_model_test):
    LR_power = random.uniform(-4.0, -2.5)
    random_learning_rate = 10 ** LR_power
    beta_power = random.uniform(-5.7, -2.3)
